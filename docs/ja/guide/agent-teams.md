...
このリポジトリは、単なる Podman ラッパーや OpenClaw チームスターターではありません。`ONI-CADIA` という AGIカントリーをシミュレーションするための構成です。リセット時、エージェントたちが覚えているのは名前だけで、人格、立ち位置、関係性は公共圏での交流から育ちます。
...
- `SOUL.md`: 名前だけを覚えている初期状態と、人格を育てるための姿勢
- `IDENTITY.md`: 覚えている名前と運用設定。固定役職は置かない
- `USER.md`: 支援対象
